...
//					if (i == ((BigTwoClient)game).getPlayerID()) {
			...
			if (
				...
				for (int i=0; i<4; i++) {
					...
					if (activePlayer != game.getCurrentIdx()) {
						if (DEBUG) {
							System.out.println("((BigTwoClient)game).getCurrentIdx() = " + ((BigTwoClient)game).getCurrentIdx());
					...
					if (((BigTwoClient)game).endOfGame()) { // if game ends 
						...
						for (int j=0; j<game.getPlayerList().get(i).getNumOfCards(); j++) {
							if (i == ((BigTwoClient)game).getPlayerID()) { // for local player
								int rank = game.getPlayerList().get(i).getCardsInHand().getCard(j).getRank();
								int suit = game.getPlayerList().get(i).getCardsInHand().getCard(j).getSuit();
					...
					// draw star
					if (i == ((BigTwoClient)game).getCurrentIdx()) {
	    ...
	    public void updatePrompt() {
	    	...
	    	if (cardIdx == null) {
		    	...
		    	Hand validHand = ((BigTwoClient)game).composeHand(currentPlayer, cardsPlayed);
		    	if (validHand == null) {
	    ...
	    public int findClickingLoaction(int x, int y) {
			...
			if (selectedCardIdx != -1) {
				if (selected[selectedCardIdx]) {
					if (y>=height*activePlayer/5+10 && y<=height*activePlayer/5+10+height/7) {
						return selectedCardIdx;
					}
					else if (y>=height*activePlayer/5+10+height/7 && y<height*activePlayer/5+30+height/7) {
					...
					if (y>=height*activePlayer/5+30 && y<=height*activePlayer/5+30+height/7) {
						return selectedCardIdx;
					} else if (y>=height*activePlayer/5+10 && y<=height*activePlayer/5+30) {
	...
	class ConnectMenuItemListener implements ActionListener {
		...
		/**
		 * This method is invoked when the Connect item in menu is clicked, it just try tto connect to the server.
		 * 
		...
		public void actionPerformed(ActionEvent e) {
			...
			} else if (((BigTwoClient) game).getPlayerID() >= 0 && ((BigTwoClient) game).getPlayerID() <= 3) {
