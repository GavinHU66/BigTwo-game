...
public class BigTwoClient implements CardGame, NetworkGame {
	...
	public void distributeCards(Deck deck) {
		...
		if (table.getActivePlayer() == table.getLocalPlayer()) {
	...
	public void checkMove(int playerID, int[] cardIdx) {
		...
		CardGamePlayer localPlayer = this.getPlayerList().get(playerID);
		...
		isLegalMove = false;
		cardsPlayed = localPlayer.play(cardIdx);
		validHand = composeHand(localPlayer, cardsPlayed);
		 ...
		 if (!isLegalMove) {
			 ...
			 if (!sayPass) { // if not say pass, then add it to handsOnTable and remove the cards from the player 
				...
				for (int i=cardIdx.length-1; i>=0; i--) { // move the later card can get rid of size problem
					localPlayer.getCardsInHand().removeCard(cardIdx[i]);
				}
			}
	...
	public synchronized void makeConnection() {
		...
		if (playerName != null && playerName.length() > 0) {
			...
			JOptionPane.showMessageDialog(null, "Name should not be empty, Connection failed");
			playerID = -1;
			table.setLocalPlayer(playerID);
		}
